...
def mse_loss(outputs, targets):
    """
    compute mse loss
	:param outputs: （N,1,H,W） aftered sigmoid
	:param targets: (N,H,W) value {0,1,...C-1}
	:return:loss
	"""
    # logits = logits.expand_as(targets)
    outputs = outputs.squeeze(1)
    # print(logits.shape)
    # nn.MSELoss 中：当logits与targets的shape不完全一致时，会通过广播机制使得两者size相匹配，
    # 虽然不会报异常，但可能导致结果错误。建议在计算loss之前使两者保持同样的shape
    return nn.MSELoss()(outputs, targets)


def mse_loss_with_sigmoid(logits, targets):
    pass
...
def dice_loss(logits, targets, smooth=1.0):
	...
	"""
    outputs = F.softmax(logits, dim=1)  # (N,C,H,W)
    targets = torch.unsqueeze(targets, dim=1)  # (N,1,H,W)      # tensor.type() 可用来改变tensor的类型
