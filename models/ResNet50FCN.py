...
def define_G(input_nc, output_nc, ngf, netG, init_type='normal', init_gain=0.02, gpu_ids=[]):
    # input_nc--3   output_nv--1   ngf--64  netG--coord_resnet50
    net = None
...
class ResNet50FCN(BaseModel):
    def __init__(self, coordconv=False, num_classes=1, pretrained_backbone=None):
        """
        ...
        if coordconv:
            ...
            self.conv_pred_2 = CoordConv2d(64, self.num_classes, kernel_size=3, padding=1)
        else:
            ...
            self.conv_pred_2 = nn.Conv2d(64, self.num_classes, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.upsample(self.relu(self.conv_fpn4(x + x_4)))  # 1/2

        # output layers
        x = self.upsample(self.relu(self.conv_pred_1(x)))
        x = self.sigmoid(self.conv_pred_2(x))    # 1  out=1
        # print(x.shape)
        return x
